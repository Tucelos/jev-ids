<!--
Upstream's README, preserved unchanged in content by the fork.
This file was `README.md` at the repository root of https://github.com/jev-ids/jev-ids until this
fork replaced that page with its own. Only the relative links below were rewritten, because the file
moved one folder down; every word, number and table is upstream's.
-->

> **This is upstream's front page, kept as it was.** The fork's own front page is
> [`../README.md`](../README.md). Every number on this page was measured by Paulo Severo, Silvio
> Quincozes and Amanda Dias, and belongs to them.

<img src="banner.svg" alt="Jev IDS banner showing the jev_ids terminal lockup, the tagline &quot;Intrusion detection with a System One Model, benchmarked against an LLM and a Random Forest.&quot;, and the command that runs a detector over NSL-KDD" width="100%" />
...
<img src="verdict.svg" alt="One flow, two typed answers: is_attack, a noul question, returns 0.82, above the 0.5 cut, so the verdict is attack; category, a choice question, returns dos with confidence 0.84" width="100%" />

Every request follows the same path:

<img src="request.svg" alt="The flow under test and the labeled examples go into one request to Jev, whose state holds instructions, columns, categories, examples and the flow, with two typed questions; the answers come back as p_attack 0.82, verdict attack, and category dos with confidence 0.84" width="100%" />
...
<img src="results-f1-three-sets.svg" alt="F1 by k for three attack sets, all with the normal flows: all attacks, known-type attacks and zero-day attacks; Gemini leads the first two, Jev leads zero-day from k = 1, the Random Forest is last on zero-day" width="100%" />
